        ...
        style={{
            ...
            <Box sx={{width:'50%'}}>
                <List subheader={
                    ...
                    {teamNames[0]} {usersList.reduce((acc, user) => user.team === true ? acc + 1 : acc, 0)} / {Math.ciel(roomSize / 2)}
                    </ListSubheader>
                }
                >
            ...
            disabled={isDisabled || usersList.reduce((acc, user) => user.team !== userTeam ? acc + 1 : acc, 0) >= Math.ciel(roomSize / 2)} 
            ...
            <Box sx={{width:'50%'}} style={{margin: '0px'}}>
                <List subheader={
                    <ListSubheader component="div" id="nested-list-subheader" sx={{ textAlign: 'center' }}>
                    {teamNames[1]} {usersList.reduce((acc, user) => user.team === false ? acc + 1 : acc, 0)} / {Math.ciel(roomSize / 2)}
                    </ListSubheader>
                }>
                    {Object.entries(usersList).map(([userId, user]) => {
                        if (!user.team) {
